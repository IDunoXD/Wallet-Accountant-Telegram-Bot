...
class BotDB:
    ...
    def records_exists(self, user_id):
        """Проверяем, есть ли юзер в базе"""
        result = self.cursor.execute("SELECT `id` FROM `records` WHERE `users_id` = ?", (self.get_user_id(user_id),))
        return bool(len(result.fetchall()))
    ...
    def change_record_type(self, user_id, record_type):
        self.cursor.execute("UPDATE `records` set `record_type` = ?, `users_id` = ? WHERE `date` = (SELECT MAX(`date`) FROM `records`) ",
            (record_type,
            self.get_user_id(user_id)))
        return self.conn.commit()

    def remove_record(self,user_id):
        self.cursor.execute("DELETE FROM `records` WHERE `date` = (SELECT MAX(`date`) FROM (SELECT * FROM `records` WHERE `users_id` = ?)) AND `users_id` = ?",
